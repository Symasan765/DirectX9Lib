...
public:
	...
	~cCamera();
	//セット系関数
	void SetCamera(COORD3D, COORD3D, COORD3D);
	...
	void PovUpVectorRotat(D3DXMATRIX);
	void PovBaseXRotation(const float);
	void SetLookPos(D3DXMATRIX);
	void SetLookPos(const float, const float, const float);
	D3DXVECTOR3 GetLookVector()const;
	D3DXVECTOR3 GetpUpVector()const;
private:
