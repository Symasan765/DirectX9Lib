...
void cCamera::Projection(const LPDIRECT3DDEVICE9 pD3dDevice){
	...
	//カメラ座標と行列を掛け合わせる
	const int bufX = machine.cameraP.cP.x && 0x80000000;
	const int bufZ = machine.cameraP.cP.z && 0x80000000;
...
void cCamera::rotation(const float bX, const float bY, const float bZ){
	...
	D3DXMatrixRotationYawPitchRoll(&mtxCamera.mtxRot, rot.y, rot.x, rot.z);
}
